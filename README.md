...
| 参数 | 详细 | 示例 |
|------------------------|----------------|----------------|
| holidaytts.DATA (**微信测试号会出现截断现象，可以使用下面的截断方案替代**) | 下一休息日综合提醒 | 还有3天就周六了，好好工作吧！距离国庆还有18天，早着呢 |
...
| 参数                       | 详细        | 示例                                                    |
|--------------------------|-----------|-------------------------------------------------------|
| note_en.DATA (**微信测试号会出现截断现象，可以使用下面的截断方案替代**)             | 金山每日一句-英文 | Nothing in this world that's worth having comes easy. |
| note_ch.DATA (**微信测试号会出现截断现象，可以使用下面的截断方案替代**)             | 金山每日一句-中文 | 这世界上凡是值得拥有的东西，都不易获得。                                  |
| one_talk.DATA (**微信测试号会出现截断现象，可以使用下面的截断方案替代**)            | 每日一言-内容   | 愿你遍布祖国山河，觉得人生也值得                                      |
| talk_from.DATA           | 每日一言-来源   | 晓良                                                    |
| earthy_love_words.DATA (**微信测试号会出现截断现象，可以使用下面的截断方案替代**)   | 土味情话(彩虹屁) | 我今晚会很忙，忙着跟你过日子                                        |
| moment_copyrighting.DATA (**微信测试号会出现截断现象，可以使用下面的截断方案替代**) | 朋友圈文案     | 错过太阳就不要再错过月亮了                                         |
| poison_chicken_soup.DATA (**微信测试号会出现截断现象，可以使用下面的截断方案替代**) | 毒鸡汤       | 我从不以强凌弱，我欺负他之前，真不晓得他比我弱。                              |
| poetry_content.DATA (**微信测试号会出现截断现象，可以使用下面的截断方案替代**)      | 古诗古文-内容   | 举头望明月，低头思故乡。                                          |
...
| 参数                           | 详细   | 示例                |
|------------------------------|------|-------------------|
| comprehensive_horoscope.DATA (**微信测试号会出现截断现象**) | 综合运势 | 太多了，不示例了，自己调用查看效果 |
| love_horoscope.DATA (**微信测试号会出现截断现象**)          | 爱情运势 | 太多了，不示例了，自己调用查看效果 |
| career_horoscope.DATA (**微信测试号会出现截断现象**)        | 事业学业 | 太多了，不示例了，自己调用查看效果 |
| wealth_horoscope.DATA (**微信测试号会出现截断现象**)        | 财富运势 | 太多了，不示例了，自己调用查看效果 |
| healthy_horoscope.DATA (**微信测试号会出现截断现象**)       | 健康运势 | 太多了，不示例了，自己调用查看效果 |
...
| birthday_message.DATA (**微信测试号会出现截断现象，可以使用下面的截断方案替代**) | 生日消息和节日消息                 | 距离 宝贝 的生日还有122天，距离 中秋节还有30天                                   |
| course_schedule.DATA (**微信测试号会出现截断现象，可以使用下面的截断方案替代**)  | 每日的课表                     | 08:00-09:35 高等数学<br/> 09:35-10:35 大学语文 <br/> 10:35-11:35 大学英语 |
