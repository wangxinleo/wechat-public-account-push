...
- [√] 现在可以在模板中获取收件人(to_name.DATA)的姓名了
- [√] 修复了地区只能指定一个的问题，现在可以分不同的人指定获取不同地区的天气
- [√] 现在大家可以使用每天不同的彩虹屁(earthy_love_words.DATA)了 感谢 @ZzqiZQute
- [√] 现在可以指定每个人推送不同的模板了，而且每个模板消息支持自定义打开一个你想要展示的网址。
- [√] 添加workflow执行指定分支脚本配置 感谢 @LordonCN
- [√] 修复github action默认使用utc时区，导致北京时间8点前的推送不显示为当天。现在会强制回显为PRC时区。
- [√] 增加推送回执，不用去看控制台咯！
- [√] 优化仓库目录结构，主脚本代码更清爽了！
- [√] 节日推送千呼万唤，现在可以根据配置就近推送前N个节日了，就算节日过了也不怕啦！
- [√] 加入了文艺青年专属每日一句！
